...
import 'package:di_art/di_art.dart' show MissingPropertyException, Properties;
...
definitionFolderTests() {
  ...
  group('`definition` folder', ()
  {
    ...
    test('can`t get nonexistent properties from a properties holder', () {
      expect(() => properties.get(nonexistentKey), allOf([throwsException, throwsA(isA<MissingPropertyException>())]));
    });

    test('can`t get properties with different type from a properties holder', () {
      expect(() => properties.get<int>(key1), allOf([throwsException, throwsA(isA<MissingPropertyException>())]));
    });
